...
heatwave_amplification <- function() {
  ...
  # Persist the per-day summer ΔT frame (with heatwave flag) so the §3.8
  # figures can be drawn without recomputing the catalogue.
...
# §3.8 -- heatwave figures: amplification lollipop + per-pair boxplots
heatwave_figures <- function() {
  ...
  dmin <- res[res$variable=="delta_T_min", ]
  dmin <- dmin[order(dmin$diff), ]
  pairs <- dmin$pair

  # Lollipop: non-HW mean (grey) vs HW mean (red), Δ + Wilcoxon p annotated
  png(file.path(FIG, "heatwave_amplification.png"), 1700, 1000, res=160)
  op <- par(mar=c(4, 11, 1.6, 1.2), mgp=c(2.6, 0.7, 0))
  y <- seq_len(nrow(dmin))
  xlim <- range(c(dmin$mean_hw, dmin$mean_nhw)) + c(-0.2, 0.3)
  plot(NA, xlim=xlim, ylim=c(0.4, nrow(dmin)+0.6), yaxt="n",
       xlab="ΔT_min (°C)", ylab="", panel.first={
         grid(nx=NULL, ny=NA, col="grey92", lty=1); abline(v=0, col="grey55", lwd=1)})
  ...
  # Boxplots: summer daily ΔT_min, non-HW vs HW, per pair (ordered by Δ)
  ...
  bx$pair  <- factor(bx$pair,  levels=pairs)
  bx$group <- factor(bx$group, levels=c("non-HW","HW"))
  n_p  <- length(pairs)
  at_v <- as.vector(rbind(seq_len(n_p)*3 - 1, seq_len(n_p)*3))
  boxplot(value ~ group + pair, data=bx,
          col=rep(c("grey85","firebrick"), n_p), names=rep("", 2*n_p),
          cex.axis=0.7, xlab="", ylab="Summer daily ΔT_min (°C)",
...
# =============================================================================
# COLLECT DOCUMENTS  (create docs/ and move all generated documents into it)
# =============================================================================
# Gathers the report sources, rendered outputs and LaTeX includes from the
# project root into a single docs/ folder. Data (data/), figures (output/)
# and this pipeline script (final_script.R) stay at the root. Safe to re-run:
# files already inside docs/ are left untouched.

DOCS <- "docs"

collect_documents <- function() {
  dir.create(DOCS, showWarnings = FALSE, recursive = TRUE)
  # Report sources/outputs to relocate (keep final_script.R itself at root).
  patterns <- c("^final_report\\.(Rmd|md|pdf|html)$",
                "^report\\.(Rmd|md|pdf|html)$",
                "^final_script\\.(Rmd|md)$",
                "^preamble\\.tex$")
  files <- unique(unlist(lapply(patterns, function(p)
    list.files(".", pattern = p, full.names = FALSE))))
  moved <- 0L
  for (f in files) {
    if (!file.exists(f)) next
    dest <- file.path(DOCS, basename(f))
    if (normalizePath(f, mustWork = FALSE) == normalizePath(dest, mustWork = FALSE)) next
    if (file.copy(f, dest, overwrite = TRUE)) { file.remove(f); moved <- moved + 1L }
  }
  message(sprintf("collect_documents(): moved %d document(s) into %s/", moved, DOCS))
}
collect_documents()
